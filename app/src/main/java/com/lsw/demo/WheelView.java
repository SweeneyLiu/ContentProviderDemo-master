...
public class WheelView extends View {
     ...
     * 默认字体颜色
     */
    private int normalColor = 0x26FFFFFF;
    /**
     * 选中时候的字体颜色
     */
     ...
     * 蒙板高度
     */
    private float maskHeight = 72.0f;
    /**
     * 选择监听
     */
    ...
    private static final int SELECT_CHANGED = 0x002;
    /**
     * 移动距离
     */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        controlWidth = getMeasuredWidth(); // getWidth();
        if (controlWidth != 0) {
    ...
    private void noEmpty() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                handler.removeMessages(SELECT_CHANGED);
                handler.obtainMessage(SELECT_CHANGED).sendToTarget();
                break;
            }
        }
    }
    ...
    private void onSelectListener() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                onSelectListener.selecting(this, item.id, item.itemText);
            }
        }
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        unitHeight = (int) attribute.getDimension(
                R.styleable.WheelView_unitHeight, 72);
        normalFont = attribute.getDimension(
                R.styleable.WheelView_normalTextSize, 24.0f);
        selectedFont = attribute.getDimension(
                R.styleable.WheelView_selectedTextSize, 24.0f);
        itemNumber = attribute.getInt(R.styleable.WheelView_itemNumber, 5);
        normalColor = attribute.getColor(
                R.styleable.WheelView_normalTextColor, 0x26FFFFFF);
    ...
    public interface OnSelectListener {
         ...
         * 结束选择
         */
        void endSelect(WheelView v, int id, String text);

        /**
         * 选中的内容
         */
        void selecting(WheelView v, int id, String text);
